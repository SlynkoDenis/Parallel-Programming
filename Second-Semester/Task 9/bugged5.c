    ...
    #pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid, i)
    {
        ...
        #pragma omp sections nowait
        {
            #pragma omp section
            {
                ...
                // Program works without this unlock (only with move in the second section),
                // however the move can be done for the faster execution start
                // it the second parallel section
                // omp_unset_lock(&locka);
            }
            ...
            #pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    a[i] += PI + i;
                omp_unset_lock(&locka);
                // moved to be executed before omp_set_lock(&locka)
                // omp_unset_lock(&lockb);
            }
        }
    }
